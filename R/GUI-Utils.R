...
help_button <- function(id, ...) {
  shiny::tags$button(
    id=id, type="button", class="btn btn-default action-button", "?",
    style="font-weight:bold;font-family: \"Times New Roman\", Times, serif; background-color:#4169E1; color: #fff;", ...
  )
}
...
info_button <- function(id, ...) {
  shiny::tags$button(
    id=id, type="button", class="btn btn-default", "i",
...
hr <- function(col, ...) {
  shiny::tags$hr(
    style=paste0(
      "width: 30px; height: auto; border: 1px solid ", col,
      "; margin: 0; margin-top: 8px; padding:0; vertical-align: middle"
    )
  )
}
