...
`[<-.jms.database.table.id` <- function(x, i=NULL, j=NULL, value) {
  ...
  if (missing(i)) {
    ...
    log.info("Adding a new table row with id %s", id, ns="jms-database.jms-database-table")
  } else {
    if (any(i < 0)) stop("Use x[-id] to remove ids")
    s <- if (length(i) == 1) "" else "s"
    log.info("Updating table row%s with id%s %s", s, s, i, ns="jms-database.jms-database-table")
    id <- i
    i <- which(ids %in% id)
  }
  ...
  log.debug(ns="jms-database.jms-database-table",
    "i=[%s], j=[%s], value=[%s]",
    paste0(i, collapse=","),
    paste0(j, collapse=","),
